...
mongoClient.connect(process.env.MONGODB_URI || ENVIRONMENT.CONNECTION, (err, client) => {
	if (err) {
...
io.on('connection', function(clientSocket) {
	...
	clientSocket.on('message', function(sent) {
		sent =JSON.parse(sent);
	
		log('User ' + colors.yellow(user.username) + 
		          " (sockID=" + user.socket.id + ') sent ' + colors.green(sent.msg))
			...
			if (!err) {
				...
				request.then((data) => {
					...
					try {
						 ...
						 response = {
							'from': user.username,
							'img':"https://www.thermaxglobal.com/wp-content/uploads/2020/05/image-not-found-300x169.jpg",
							'message': sent.msg,
							'image_sent':sent.img,
							'date': new Date()
						}
